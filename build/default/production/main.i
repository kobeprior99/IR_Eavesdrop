...
void main(void)
{
    ...
    for (;;)
    {
        ...
        if (PORTAbits.RA2 == 0 && bufferReady == 0)
        {
            capture = 1;
            DispReceive();
        }
...
void myTMR3ISR(void)
{
    ...
    case TX_IDLE:
        ...
        if (transmit == 1)
        {
            tmr3ISRstate = TX_TRANSMIT;
        }
        else if (capture == 1)
        {
            tmr3ISRstate = TX_WAIT_PULLDOWN;
    ...
    TMR3_WriteTimer(0x10000 - (1666 - TMR3_ReadTimer()));
    PIR2bits.TMR3IF = 0;
}
...
void setSample(uint16_t index, uint8_t value)
{
    ...
    if (value == 1)
    {
        recieve_buffer[byteIndex] |= (1 << bitIndex);
    }
    else
    {
        recieve_buffer[byteIndex] &= ~(1 << bitIndex);
