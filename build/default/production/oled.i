...
void Oled_Contrast(U8 cmd) {
 if(cmd == 0x01 && Display.Contrast < 255)
 {
  Display.Contrast++;
 }
 if(cmd == 0x00 && Display.Contrast > 1)
 {
  Display.Contrast--;
 }
 Oled_SetContrast();
    ...
    for(j = 0; j < Font.Height; j++)
    {
        for(i = 0; i < Font.Width + Font.LetterSpacing; i++)
        {
            if(i < Font.Width)
            {
                ...
                {
                    Oled_Pixel(x + i, y + j, 0x01);
                } else
                {
                    Oled_Pixel(x + i, y + j, 0x00);
                }
